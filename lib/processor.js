...
class Processor {
  ...
  async message(text, provider, roomId, eventId, userId, eventTime, classifierName, language, personality) {
    ...
    if(classification) {
      ...
      if(positiveResults.length) {
        ...
        this.challenge[roomId] = message;
      } else {
        this.clean[roomId] = message;
      }
    }
  ...
  async dialogue(text, provider, roomId, eventId, userId, classifierName, language, personality) {
    ...
    console.log('lastchallenge: ', this.challenge[roomId]);
    for(let type in ['challenge', 'clean'])
      if(this[type][roomId]) {
        context.parameters[type + 'Id'] = this[type][roomId].id;
        context.parameters[type + 'Message'] = this[type][roomId].text;
        context.parameters[type + 'User'] = this[type][roomId].user
      }
    ...
    return new Promise((resolve, reject) => {
      request.on('response', async response => {
        ...
        const action = response.result.action.match(/smalltalk.*/, 'smalltalk');
        const emit = sm.action(action);

        var res = {
          status: 'OK',
          smalltalk: action == 'smalltalk',
          inDialog: emit != null && emit.length,
          response: (smalltalk) ? response.result.fulfillment.speech : lp.response(emit)
        }
