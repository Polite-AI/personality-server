...
class Database {
  ...
  async getRoom(room) {
    ...
    const exists = new ps('get-room', 'SELECT room_id, provider, provider_id, initialised, type, owner, dialogstate, key, time FROM rooms WHERE provider=$1 AND provider_id=$2 ORDER BY time DESC')
  ...
  async updateRoom(room, property) {
    await this.initialised;
    let value = (property == 'dialogState')?JSON.stringify(room[property]):room[property];
    return await this.db.none('UPDATE rooms SET $2:raw=$3 WHERE room_id=$1', [room.id, property, value])
