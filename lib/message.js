...
class Room {
  ...
  /**
   * @type {Promise} Resolves to true if room exists
   */
  get exists() {
    if(!this.id)
   ...
   * @param {number} flags.offset=0 start at this offset in the list (skip offset-1 entries)
   * @param {number} flags.limit=50 return limited number of records (default is 10)
   * @param {bool} flags.reverse=false list returned in most recent first date order (default is false)
   * @return  {Promise}  Resolves to an array of matching message objects
   */
  getList(flags) {
    return messageGet(null, this, flags)
  }
...
class Message {
   ...
   * @return {Promise}               Resolves when saved to database
   */
   ...
   * @return {Promise}      Resolves when appeal is stored
   */
